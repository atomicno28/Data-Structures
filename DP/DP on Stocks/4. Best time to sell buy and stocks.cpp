...
int tabul(vector<int>&prices, int k)
{
   ...
   for(int i=prices.size()-1;i>=0;i--)
   {
    for(int buy=0;buy<2;buy++)
    {
        for(int cap=1;cap<k+1;cap++)
        {
            ...
            else dp[i][buy][cap] = max(prices[i]+dp[i][1][cap-1],0+dp[i+1][0][cap]);
        }
    }
   }
   return dp[0][1][2];  
}
int spaceOptimise(vector<int>& prices, int n, int k)
{
...
int main()
{
    ...
    // cout << recurs(0,1,k,prices);

    // vector<vector<vector<int>>> dp (n,vector<vector<int>>(2,vector<int>(k+1,-1)));
    // cout << memoiz(0,prices,1,k,dp);

    // cout <<tabul(prices,k);

    cout << spaceOptimise(prices,prices.size(),k);
